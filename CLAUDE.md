...
- **Monitor has exactly three jobs: log, report, and track tasks.** It never detects a project's language, guesses build/test commands, or otherwise inspects what the project does — that's guessing, not recording. `monitor/profile.json` only auto-fills the project's directory name (for branding pages) plus the report KPI list; that's the entire "source of truth." If an agent needs real project orientation, that's what companion skills like `graphify` are for (see below), not monitor. The log schema lives in code in `logger.py` (`REQUIRED`/`LEVELS`/`STATUSES`), identical across every project — the same for a brand-new project as an old one. `/monitor:init` seeds the profile; `/monitor:update` reconciles it.
...
- This repo's own `monitor/` data folder (profile.json, logs, reports, tasks) is committed like any consumer project's — logs and reports are how a future agent (or teammate) recovers context, which only works if they're actually in the repo. The installed engine copy under `.claude/skills/monitor/` and `.claude/commands/monitor/` stays gitignored — it's a project-local install of the portable source already shipped under `plugins/monitor/`, re-synced via `install-monitor.sh` rather than tracked as a second copy.
...
stated above under "Editing the engine." Rules for using it live in
`.claude/skills/monitor/SKILL.md` — read it
