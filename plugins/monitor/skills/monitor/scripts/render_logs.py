...
N>1 is logs/page-N.html. Stale page files from a shrunk log (after
/monitor:clean-logs) are removed. Called by logger.py after every entry; also
runnable standalone.
...
def build_html(page_entries: list[dict], brand: str, branch: str, *, total: int,
               counts: dict, n_frags: int, last: str, page_num: int,
               total_pages: int) -> str:
  ...
  <fieldset class="filter" aria-label="Filter by status">
    ...
    if page_entries:
        body = '  <div class="log">\n' + "\n".join(_card(e) for e in page_entries) + "\n  </div>"
    else:
...
def _prune_stale_pages(logs_dir: Path, total_pages: int) -> None:
    for f in logs_dir.glob("page-*.html"):
...
def render(root: Path) -> Path:
    ...
    real = [e for e in all_entries if e.get("fragment") is None]
    frags = [e for e in all_entries if e.get("fragment") is not None]
    total = len(real)
    ...
    for page_num in range(1, total_pages + 1):
        ...
        page_entries = all_entries[start:start + mlib.PAGE_SIZE]
        html = build_html(page_entries, brand, branch, total=total, counts=counts,
                          n_frags=len(frags), last=last, page_num=page_num,
                          total_pages=total_pages)
        (logs_dir / mlib.page_filename(page_num)).write_text(html, encoding="utf-8")

    _prune_stale_pages(logs_dir, total_pages)
