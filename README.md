...
[![Version](https://img.shields.io/badge/version-1.12.1-blue)](plugins/monitor/.claude-plugin/plugin.json)
...
Monitor gives any project a local `monitor/` folder — a **Dashboard** linking a **Reports** page (one self-contained HTML report per task or change), a **Logs** page (a canonical, append-only text log rendered to filterable, paginated HTML), and a **Tasks** page (lifecycle-tracked units of work with self-reported token/credit/cost metrics). It's a Claude Code plugin, but the engine underneath is plain Python 3 stdlib with zero dependencies — no `pip install`, no CDN assets, no server.
...
> [!NOTE]
> Monitor's job: **log**, **report**, and **track tasks**. It never detects a project's language, guesses build/test commands, or otherwise inspects what a project does — that's guessing, not recording. If an agent needs real project orientation, that's what a companion skill like `graphify` is for.
...
- **Dashboard** (`monitor/index.html`) — links Reports, Logs, and Tasks, shows the current branch and the open-task count.
...
- **Tasks** (`monitor/tasks/`) — a canonical `tasks.mtr` of task-start/update/close events, rendered to one paginated card per task: current status, aggregated metrics, and an oldest-to-newest timeline. A task is a tracked entity of its own, cross-referenced from log entries via `--task-id`.
