...
A Claude Code plugin marketplace distributing **`monitor`** — a portable logging + reporting workflow that gives any project a local `monitor/` folder (a **Dashboard** linking **Reports** and **Logs** pages), driven by a project-specific log schema and report template that evolve additively. The engine is **stdlib-only Python 3** — no pip installs, no external assets, no hardcoded paths.
...
- **Backward-compatible by construction.** `monitor/profile.json` is the single source of truth; reconciling it only ever adds fields, never removes or renames — so schema and template upgrades never break old data.
...
- **Logs** (`monitor/logs/`) — a canonical, newest-first `operations.log`, validated against a project-specific `schema.json` and rendered to a filterable `index.html`. Never hand-edited — always written through `logger.py`.
